...
export class AuthService {
  ...
  sign_in(): Observable<any> {
    ...
    let toto;
    let req = this.obtainAccessToken();
    req.subscribe(() => {
  ...
  obtainAccessToken() {
    ...
    let oauth2_token_endpoint = 'https://born-to-be-alive-api.herokuapp.com/oauth/token?grant_type=password&username=fabiendubois&password=jwtpass';
